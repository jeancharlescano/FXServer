...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
			...
			align    = 'top-left',
			elements = {
				{label = _U('producter_clothes_civil'), value = 'citizen_wear'},
				{label = _U('producter_clothes_producter'), value = 'producter_wear'},
			},
		}, function(data, menu)
			...
			if data.current.value == 'producter_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'producter_actions_menu'
...
function OpenProducterActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'producter_actions', {
			title    = 'Producter',
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'producter', function(data, menu)
					menu.close()
				end)
			end
		end, function(data, menu)
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'producter', vehicleProps)
				end, function(data, menu)
...
function OpenMobileProducterActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_producter_actions', {
			title    = 'producter',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_producter', 'Producteur', amount)
								end)
							end
						end
					end, function(data, menu)
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_producterjob:getStockItems', function(items)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_producterjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_producterjob:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_producterjob:putStockItems', itemName, count)
						end
					end, function(data2, menu2)
						menu2.close()
					end)
...
AddEventHandler('esx_producterjob:hasEnteredMarker', function(zone)
	if zone == 'PotatoFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'producter'  then
		CurrentAction     = 'potato_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementPotato' and PlayerData.job ~= nil and PlayerData.job.name == 'producter'  then
		CurrentAction     = 'potato_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'producter'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'ProducteurActions' and PlayerData.job ~= nil and PlayerData.job.name == 'producter' then
		CurrentAction     = 'producter_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'producter' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'producter' then
...
AddEventHandler('esx_producterjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'PotatoFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'producter' then
		TriggerServerEvent('esx_producterjob:stopHarvest')
	end  
	if (zone == 'TraitementPotato') and PlayerData.job ~= nil and PlayerData.job.name == 'producter' then
		TriggerServerEvent('esx_producteurjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'producter' then
		TriggerServerEvent('esx_producterjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.ProducteurActions.Pos.x, Config.Zones.ProducteurActions.Pos.y, Config.Zones.ProducteurActions.Pos.z)
		...
		AddTextComponentString("Producteurs")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'producter' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'producter' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_producterjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_producterjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  38) and PlayerData.job ~= nil and PlayerData.job.name == 'producter' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'potato_harvest' then
					TriggerServerEvent('esx_producterjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'potato_traitement' then
					TriggerServerEvent('esx_producterjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_producterjob:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'producter_actions_menu' then
					OpenProducterActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'producter', vehicleProps)
					end
		...
		if IsControlPressed(0,  167) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'producter' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobileProducterActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)